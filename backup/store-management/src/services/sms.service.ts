...
export class SmsService {
  ...
  /**
   * Interpolates dynamic placeholders in SMS templates
   */
  ...
  /**
   * Sends an SMS to a Bangladeshi recipient
   */
  static async sendSms(options: SendSmsOptions) {
    ...
    // 3. Built-in Sandbox / Simulator Mode (When API Key is pending or testing)
    await prisma.smsLog.create({
      data: {
        recipientPhone: cleanPhone,
        message: options.message,
  ...
  /**
   * Fetches live SMS balance from configured provider
   */
  static async checkBalance() {
    ...
    if (!apiKey) {
      return {
        balance: 100,
        formatted: '100 SMS (Sandbox Simulator)',
        currency: 'SMS',
      };
    }
  ...
  /**
   * Trigger: Order Shipped / Dispatched
   */
