...
import { productsTable, categoriesTable, productImagesTable } from '../../db/schema.js';
...
export async function createProduct(req: AuthRequest, res: Response) {
  try {
    const {
      title, slug, categoryId, price, compareAtPrice, sku, stock,
      shortDescription, description, badge, tags = [], isFeatured = false,
    ...
    if (!catId) {
      const firstCat = await rdb().select({ id: categoriesTable.id }).from(categoriesTable).limit(1).then(r => r[0] ?? null);
      catId = firstCat?.id;
    }
    ...
    const created = await db.transaction(async (tx) => {
      ...
      if (images.length > 0) {
        await tx.insert(productImagesTable).values(
          images.map((url: string, idx: number) => ({ productId: product.id, url, position: idx }))
        );
      }
