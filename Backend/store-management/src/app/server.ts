...
import { startConsumers } from './tasks/consumers.js';
...
const start = async () => {
  ...
  // Start all background consumers (RabbitMQ + Kafka)
  await startConsumers();

  const server = app.listen(PORT, () => {
    infoLogger.info(`[Store Management Service] running on port ${PORT}`);
