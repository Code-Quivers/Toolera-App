    ...
    <div className="bg-slate-50 min-h-screen py-8 sm:py-12 border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 mb-8 flex flex-wrap items-center justify-between gap-4 shadow-xs">
          ...
          <div className="flex items-center gap-2 text-xs sm:text-sm">
            ...
            <label htmlFor="sort" className="text-slate-500 font-medium hidden sm:inline">Sort by:</label>
            <select
              id="sort"
              value={selectedSort}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="hidden lg:block space-y-6 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs h-fit sticky top-24">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              ...
              <button onClick={resetFilters} className="text-xs text-teal-600 hover:text-teal-800 font-semibold">
                Reset All
              </button>
            </div>
            ...
            <div>
              ...
              <div className="space-y-1.5 text-xs">
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-xl transition ${selectedCategory === "all" ? "bg-teal-50 text-teal-900 font-bold" : "text-slate-600 hover:bg-slate-50 font-medium"}`}
                >
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl transition flex justify-between items-center ${selectedCategory === cat.slug ? "bg-teal-50 text-teal-900 font-bold" : "text-slate-600 hover:bg-slate-50 font-medium"}`}
                  >
                    <span>{cat.name}</span>
            ...
            <div className="pt-4 border-t border-slate-100">
              ...
              <input
                type="range" min="500" max="4500" step="100" value={maxPrice}
                onChange={(e) => setMaxPrice(Number(e.target.value))}
                className="w-full accent-teal-600 cursor-pointer"
              />
              <div className="flex justify-between text-[11px] text-slate-400 mt-1">
                <span>৳500</span><span>৳4,500</span>
              </div>
            </div>

            <div className="pt-4 border-t border-slate-100">
              <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2.5">Rating</h4>
              <div className="space-y-1 text-xs">
                {[0, 4.5, 4.8].map((ratingVal) => (
                  <button
                    key={ratingVal}
                    onClick={() => setMinRating(ratingVal)}
                    className={`w-full text-left px-3 py-1.5 rounded-lg transition ${minRating === ratingVal ? "bg-teal-50 text-teal-900 font-bold" : "text-slate-600 hover:bg-slate-50"}`}
                  >
      ...
      {mobileFilterOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs" onClick={() => setMobileFilterOpen(false)} />
          <div className="relative w-4/5 max-w-xs bg-white h-full shadow-2xl z-10 flex flex-col justify-between overflow-y-auto p-6">
              ...
              <div className="mb-6">
                ...
                <div className="space-y-1 text-xs">
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl ${selectedCategory === "all" ? "bg-teal-50 text-teal-900 font-bold" : "text-slate-700"}`}
                  >
                    All Categories
                  </button>
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl ${selectedCategory === cat.slug ? "bg-teal-50 text-teal-900 font-bold" : "text-slate-700"}`}
                    >
                      {cat.name}
                    </button>
                  ))}
            ...
            <div className="pt-4 border-t border-slate-100 space-y-2">
              ...
              <button onClick={resetFilters} className="w-full py-2 text-center text-xs text-slate-500 font-medium">
                Reset
              </button>
            </div>
          </div>
