...
export function setSkipPaymentPending(pending: boolean) {
  try {
    if (pending) {
...
export function useStore() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const isSkipped = localStorage.getItem("toolera_payment_pending") === "true";
      setSkipFlag(isSkipped);
    } catch {}
  ...
  const isPaymentPending =
    skipFlag ||
    (store?.subscription
      ? ["PENDING", "UNPAID", "CANCELLED", "EXPIRED"].includes(store.subscription.status)
      : false);
