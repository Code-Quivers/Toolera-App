...
import { LogOut, User, KeyRound, Lock, AlertTriangle } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { isAdminAuthenticated, adminUser, logout, checkSession } = useAdminAuthStore();
  ...
  // Check admin authentication on every route change
  useEffect(() => {
  ...
  // If payment is pending and merchant tries to access a locked page, redirect to billing
  useEffect(() => {
  ...
  const [seenCustomersCount, setSeenCustomersCount] = useState<number>(0);

  // Initialize seen counts from localStorage
  useEffect(() => {
  ...
  // When admin navigates to /admin/orders, mark current orders as seen
  useEffect(() => {
    if (pathname === "/admin/orders") {
      setSeenOrdersCount(orders.length);
      if (typeof window !== "undefined") {
        localStorage.setItem("admin_seen_orders_count", String(orders.length));
      }
    }
  }, [pathname, orders.length]);

  // When admin navigates to /admin/customers, mark current customers as seen
  useEffect(() => {
    if (pathname === "/admin/customers") {
      setSeenCustomersCount(customers.length);
      if (typeof window !== "undefined") {
        localStorage.setItem("admin_seen_customers_count", String(customers.length));
      }
    }
  }, [pathname, customers.length]);

  const newOrdersCount = Math.max(0, orders.length - seenOrdersCount);
  const newCustomersCount = Math.max(0, customers.length - seenCustomersCount);
  ...
  const commandItems = [
    ...
    { title: "Feature Flags", category: "Settings", href: "/admin/settings/features" },
  ...
  // If session is verifying, render secure loading shield
  if (!authChecked) {
    return (
      ...
      >
        <div>
          ...
          <div className="px-3 py-4 space-y-4 overflow-y-auto max-h-[calc(100vh-140px)] text-xs">
            ...
            {/* Locked Navigation Box when payment is skipped */}
          ...
          <div className="relative w-full max-w-xl bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center gap-3">
              ...
              <input
                type="text"
                autoFocus
                placeholder="Type a command or page (e.g. 'add product', 'homepage', 'orders')..."
                value={commandQuery}
            ...
            <div className="max-h-72 overflow-y-auto p-2 space-y-1 divide-y divide-slate-100 text-xs">
              {filteredCommands.length > 0 ? (
                ...
                <div className="p-4 text-center text-slate-400 text-xs">
                  No matching admin actions found.
                </div>
              )}
            </div>
          </div>
