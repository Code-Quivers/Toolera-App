...
export function BestSellers({ settings }: BestSellersProps) {
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const { products } = useProductStore();

  const title = settings?.title || "Best Sellers";
  const subtitle = settings?.subtitle || "Top-rated China products loved by thousands across Bangladesh.";
  const layout = settings?.layout || "grid";
  const categoryFilter = settings?.category || "all";
  const source = settings?.source || "best-sellers";
  ...
  return (
    <section className="py-12 sm:py-16 bg-slate-50/60 border-b border-slate-200/60 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              href={`/shop${categoryFilter !== "all" ? `?category=${categoryFilter}` : "?filter=best-sellers"}`}
              className="flex items-center gap-1 text-xs font-bold text-amber-700 hover:text-amber-900 transition mr-1 sm:mr-2"
            >
